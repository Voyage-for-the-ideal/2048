...
`seed 200-249`），从结构上修复了蛇形特征抑制大块合并的问题。2026-08-18：
下方 ablation 表明 snake=0 在各项指标全面胜出，故成为默认权重（`seed
200-249`：mean 43007、2048=82%、4096=36% —— 对比 snake=2 的 24763 /
50% / 6%）。表中数值来自真实运行，绝不虚构。）

### Ablation 消融实验（2026-08-18）

每组将某一权重置零，15 局/组，`seed 200-214`，d4 b100 c8。`snake 0,
maxTile 15, score 2` 已成为当前默认权重；mono=0 明显有害（保留）。

| 变体 | 平均分 | 2048 | 4096 |
| --- | ---: | ---: | ---: |
| 基线（snake=2） | 24299 | 53.3% | 6.7% |
| **snake=0（新默认）** | **39663** | **66.7%** | **26.7%** |
